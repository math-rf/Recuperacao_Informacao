...
def modelo_bm25(corpus_strings, consulta_string, k1=1.5, b=0.75):
    ...
    # 2. Tokeniza a consulta usando as mesmas regras do vectorizer
    tokens_consulta = vectorizer.build_analyzer()(consulta_string)
    
    # 3. Cálculo Explícito do BM25
    ranking = []
    for i in range(N):
        score_doc = 0
...
def executar_experimentos(del_stopwords=False, use_stemming=False):
    # 1. Carrega o dataset Cranfield
    dataset = ir_datasets.load("cranfield")
    
    # 2. Prepara o Corpus (Documentos)
    # Mantemos uma lista de strings para o Modelo Vetorial e IDs para mapeamento
    doc_ids = []
    corpus_strings = []
    ...
    # 3. Prepara os Julgamentos de Relevância (Qrels)
    qrels = {}
    for qrel in dataset.qrels_iter():
        if qrel.query_id not in qrels:
            qrels[qrel.query_id] = []
            
        # REGRA DO PDF: Relevância >= 1, mas valor 1 é NÃO relevante.
        # Portanto, consideramos apenas relevância > 1.
        if qrel.relevance > 1:
            qrels[qrel.query_id].append(qrel.doc_id)

    # 4. Estruturas para guardar os resultados
    resultados_vetorial = {}
    resultados_bm25 = {}
    
    # 5. Loop pelas Consultas
